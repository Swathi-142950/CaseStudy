...
public class WashDetailsController {
	...
	@PostMapping("/saveWashPackages")
	public ResponseEntity<List<WashPackageDto>> saveWashDetails(@RequestBody List<WashPackageDto> washPackageList) throws CaseStudyException {
		List<WashPackageDto> washList = washDetailsOp.saveWashPackages(washPackageList);
		return new ResponseEntity<>(washList, HttpStatus.OK);
	}
}